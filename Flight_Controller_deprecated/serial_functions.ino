void print_serial()
{
	...
	Serial.println("max_current_altitude is: " + String(max_current_altitude));
	Serial.println("min_current_altitude is: " + String(min_current_altitude));
	Serial.println("MIN_THROTTLE_OUTPUT is: " + String(MIN_THROTTLE_OUTPUT) + ", MAX_THROTTLE_OUTPUT is: " + String(MAX_THROTTLE_OUTPUT));

	for (int i = 0; i < NUM_OF_THROTTLE_CHANNELS; ++i)
	{
	...
	{
		Serial.print("Motor " + String(i + 1) + ": " + String(all_motors[i].get_speed()));
		Serial.println(", Pin Number: " + String(all_motors[i].get_pin()));
	}
