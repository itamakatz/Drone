/*
FreeSixIMU_Teensy3.cpp - A libre and easy to use orientation sensing library for Arduino
...
#include <inttypes.h>
//#define DEBUG
#include "FreeSixIMU_Teensy3.h"
// #include "WireUtils.h"
#include "DebugUtils_Teensy3.h"

// for the new invSqrt. maybe not needed since it compiles without it (for teensy 3.6) 
#include <stdint.h>
#include <string.h>

FreeSixIMU_Teensy3::FreeSixIMU_Teensy3() {
	acc = ADXL345();
	gyro = ITG3200_Teensy3();
...
void FreeSixIMU_Teensy3::init() {
	init(FIMU_ACC_ADDR, FIMU_ITG3200_Teensy3_DEF_ADDR);
}

void FreeSixIMU_Teensy3::init(int acc_addr, int gyro_addr) {
	delay(5);
...
//
//=====================================================================================================
void FreeSixIMU_Teensy3::AHRSupdate(float gx, float gy, float gz, float ax, float ay, float az) {
	float recipNorm;
...
void FreeSixIMU_Teensy3::getQ(float * q) {
	float val[6];
	getValues(val);
...
void FreeSixIMU_Teensy3::getEuler(float * angles) {
...
void FreeSixIMU_Teensy3::getAngles(float * angles) {
	float a[3]; //Euler
	getEuler(a);
...
void FreeSixIMU_Teensy3::getYawPitchRoll(float * ypr) {
	float q[4]; // quaternion
